  ...
  return (
    <div className="grid h-full min-h-0 content-center gap-10 lg:gap-14 lg:grid-cols-[minmax(0,1.15fr)_minmax(16rem,0.85fr)]">
      ...
      <div className="order-2 flex flex-col justify-center gap-8 lg:order-1">
        ...
        <div className="animate-m3-reveal-up">
          <span className="m3-label">
            <span className="material-symbols-rounded text-[16px]">waving_hand</span>
            {hero.greeting}
          </span>
        </div>
        ...
        {/* Stats row */}
        <div className="animate-m3-reveal-up [animation-delay:0.15s] grid grid-cols-1 gap-4 sm:grid-cols-3">
          {hero.stats.map((stat) => (
            <div
              key={stat.label}
              className="m3-card-outlined flex items-center gap-4 px-5 py-4"
              style={{ borderRadius: "var(--m3-shape-large)" }}
            >
              <span
                className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full-m3 text-m3-primary"
                style={{ background: "color-mix(in srgb, var(--m3-primary) 12%, transparent)" }}
              >
                <span className="material-symbols-rounded text-[22px]">{stat.icon}</span>
              </span>
              <div>
                <p className="m-0 text-[1.25rem] font-medium text-m3-on-surface" style={{ fontFamily: '"Outfit", sans-serif' }}>
                  {stat.value}
                </p>
                <p className="m-0 text-[0.8125rem] text-m3-on-surface-variant">
                  {stat.label}
                </p>
              </div>
            </div>
      ...
      <div className="relative order-1 flex items-center justify-center animate-m3-reveal-up [animation-delay:0.1s] lg:order-2">
        <img
          className="w-full max-w-md lg:max-w-none object-contain animate-m3-float drop-shadow-[0_20px_60px_rgba(168,199,250,0.15)]"
